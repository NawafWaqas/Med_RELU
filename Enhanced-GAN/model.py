...
class Model(object):
    ...
    def make_train_op(self, images):
        ...
        elif self.cfg.loss_mode == 'wgan':
            d_loss, g_loss = losses.wgan_loss(d_real, d_fake)
        elif self.cfg.loss_mode == 'lsun':
            d_loss, g_loss =losses.js_loss(d_real, d_fake)
            ...
            for i in range(self.cfg.n_iters):
                ...
                if global_step % save_period == 0:
                    ...
                    if self.cfg.save_images:
                        ...
                        gen_images1 = np.transpose(gen_images, (1, 2, 0, 3))
                        nrrd.write(f'knee{i}.nrrd',gen_images1)
                        print("gen shape is ", gen_images1.shape)
                        num_splits=1
                        scores = []
                        for i in range(num_splits):
                            istart = i * gen_images.shape[0] // num_splits
                            iend = (i + 1) * gen_images.shape[0] // num_splits
                            part = gen_images[istart:iend, :]
                            kl = part * \
                                 (np.log(part) - np.log(np.expand_dims(np.mean(part, 0), 0)))
                            kl = np.mean(np.sum(kl, 1))
                            scores.append(np.exp(kl))
                        print( "np new metric is ",np.mean(scores), np.std(scores))
                        #print("gen image is ", gen_images)
                        #f1 = f1_score(gen_images, image_batch)
                        #print('F1 score: %f' % f1)
                        plt.figure(figsize=(100, 100))
                        grid = image_loader.grid_batch_images(gen_images)
                        p_y = expand_dims(grid.mean(axis=0), 0)
                        eps=1E-16
                        kl_d =grid * (log(grid + eps) - log(p_y + eps))
                        sum_kl_d = kl_d.sum(axis=1)
                        avg_kl_d = mean(sum_kl_d)
                        is_score = exp(avg_kl_d)
                        is_avg, is_std = mean(is_score), std(is_score)
                        print("is_score is is ", is_score)
    ...
    def generate_images(self, model, batch_z=None, alpha=0.):
        ...
        batch_size = 160  # self.cfg.batch_size
        z_dim = self.cfg.z_dim
        if batch_z is None:
            batch_z = np.random.normal(0, 1, size=(batch_size, z_dim))
